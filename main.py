...
class TestUrbanRoutes:
    ...
    @classmethod
    def setup_class(cls, DesiredCapabilities=None):
        # do not modify - we need additional logging enabled in order to retrieve phone confirmation code
        from selenium.webdriver import pages
        capabilities = DesiredCapabilities.CHROME
        capabilities["goog:loggingPrefs"] = {'performance': 'ALL'}
        cls.driver = webdriver.Chrome()
...
class TestUrbanRoutes:
    ...
    def test_select_supportive_plan(self):
        ...
        plan_element = self.pages.driver.find_element(*self.pages.SUPPORTIVE_PLAN_LOCATOR)
        assert "active" in plan_element.get_attribute("class"), \
        "Supportive plan should be selected but is not"
